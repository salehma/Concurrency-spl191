...
public abstract class MicroService implements Runnable {
    ...
    protected MessageBusImpl MessageBus_;
    private ConcurrentHashMap<String, Callback<? extends Event>> Event_callback_list;
    private ConcurrentHashMap<String, Callback<? extends Broadcast>> Broadcast_callback_list;
    private ConcurrentHashMap<Event, Callback> OnComplete_list;
    ...
    public MicroService(String name) {
        this.name = name;
        MessageBus_ = MessageBusImpl.getInstance();
        Event_callback_list = new ConcurrentHashMap<>();
        Broadcast_callback_list = new ConcurrentHashMap<>();
        OnComplete_list = new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        Event_callback_list.put(type.getName(), callback);
        MessageBus_.subscribeEvent(type, this);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        Broadcast_callback_list.put(type.getName(), callback);
        MessageBus_.subscribeBroadcast(type, this);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
      //  OnComplete_list.put(e, onComplete);
        return MessageBus_.sendEvent(e);
    }
    ...
    public final void run() {
        ...
        Callback call_ = null;
        Message m;
        MessageBus_.register(this);
        initialize();
        while (!terminated) {
            try {
                m = MessageBus_.awaitMessage(this);

                if (m instanceof Event)
                    call_ = Event_callback_list.get(m.getClass().getName());
                else if (m instanceof Broadcast)
                    call_ = Broadcast_callback_list.get(m.getClass().getName());
